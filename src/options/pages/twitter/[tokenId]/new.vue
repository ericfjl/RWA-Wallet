...
const doSubmit = async () => {
  ...
  if (enableOneTimePayment === true) {
    ...
    const tx = await contractWrite(
      "BuidlerProtocol",
      "addOTP",
      parseEther(oneTimePaymentBasicPrice),
      inviteCommission,
  ...
  const { error: dbError } = await supabase
    .from("item")
    .insert({ metadata: sbtMetadata, chain, address, tokenid, tokentype, appaddress, itemid: nextItemId, itemtype: itemType });
  if (dbError) {
    error = dbError;
  ...
  alertSuccess(
    "Store data into database successed!",
    async () => {
      isLoading = false;
