...
<template>
  <div class="space-y-6 text-white">
    <div text-lg>
      <p text-center>Write down this 12-word Secret Recovery Phrase and save it in a place that you trust and only you can access.</p>
      <br />
      <br />
    ...
    <div flex justify="between" items-center text-indigo-500 text-lg>
      <button v-if="isShow" flex items-center @click="isShow = false"><material-symbols-disabled-visible mr-2 /> Hide Recovery Phrase</button>
      <button v-else flex items-center @click="isShow = true"><ic-baseline-remove-red-eye mr-2 /> Reveal Recovery Phrase</button>
      <span v-if="copied" flex items-center text-green-5>
        <material-symbols-check-circle mr-2 />
        Copied.
      </span>
      <button v-else flex items-center @click="copy(mnemonicRealStr)"><ion-ios-copy mr-2 />Copy to clipboard</button>
    </div>
    <div>
