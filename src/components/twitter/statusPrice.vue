...
const stats = $computed(() => {
  return [
    { id: 1, name: "Sold / Max Supply", value: `${nft.totalSupply} / ${nft.maxSupply}` },
  ...
  <div class="bg-gray-900 py-24 sm:py-32">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      <dl class="text-center grid gap-x-8 gap-y-16 grid-cols-1 lg:grid-cols-3">
        <div v-for="stat in stats" :key="stat.id" class="flex flex-col mx-auto max-w-xs gap-y-4">
          <dt class="text-base text-gray-400 leading-7">{{ stat.name }}</dt>
          <dd class="order-first font-semibold text-white tracking-tight text-3xl sm:text-5xl">
            <template v-if="typeof stat.value === 'string'">
              {{ stat.value }}
            </template>
            <template v-else>
              <vue3-autocounter :startAmount="0" :endAmount="stat.value" :duration="3" prefix="" suffix="" separator="" decimalSeparator="" :decimals="0" :autoinit="true" />
            </template>
          </dd>
        </div>
      </dl>
      <div class="mx-auto max-w-md space-y-4 mt-20 lg:space-y-0 lg:max-w-5xl lg:grid lg:gap-5 lg:grid-cols-2">
        <div v-for="tier in tiers" :key="tier.name" class="rounded-lg flex flex-col shadow-lg overflow-hidden">
          <div class="bg-white py-8 px-6 sm:p-10 sm:pb-6">
            ...
            <div class="flex font-bold mt-4 tracking-tight text-6xl items-baseline">
              {{ tier.nftCount }}
