...
const tagsWithCharCodeNumberSum = tags.map((tag) => {
  const sum = tag.split("").reduce((acc, val) => acc + val.charCodeAt(0), 0) % 10;
...
<template>
  <div gap-x-2 flex>
    <span class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium" v-for="item in tagsWithCharCodeNumberSum" :class="{
