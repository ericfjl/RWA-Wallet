...
const { title, size, modelValue } = defineProps<Props>();
    ...
    <div class="rounded-full" :class="{ 'h-60 w-60': isMedium, 'h-30 w-30': isSmall }">
      <DropZone v-slot="{ dropZoneActive }" class="drop-area" @files-dropped="addFilesAndStartUpload">
        <div v-if="modelValue" class="relative">
          <div
            v-show="isLoading"
            class="bg-black rounded-full flex w-full opacity-80 top-0 left-0 justify-center items-center absolute"
            :class="{ 'h-60 p-10': isMedium, 'h-30 p-5': isSmall }"
          >
          ...
          <div
            ...
            class="text-gray-400 absolute hover:cursor-pointer"
            :class="{ 'top-2 right-2 w-8 h-8': isMedium, 'top-0 right-0 w-5 h-5': isSmall }"
            @click="resetStatus"
          />
        </div>
        <div
          v-else
          class="border-dashed rounded-full flex border-2 border-gray-300 mt-1 justify-center relative overflow-hidden"
          :class="{ 'h-60 w-60 px-6 pt-5 pb-6': isMedium, 'h-30 w-30 text-xs p-2': isSmall }"
        >
          <div class="flex flex-col space-y-1 text-center py-10 justify-center items-center">
            ...
            <div class="h-10">
              <div v-if="dropZoneActive" class="text-gray-600">Drop to upload</div>
              <div v-else>
                <div class="font-bold text-center text-gray-600">Drag and Drop</div>
                <p class="text-gray-500">PNG, JPG, GIF up to 10MB</p>
              </div>
            </div>
          </div>
